# ResumeFit Agent — Product Requirements (PRD)
...
**Acceptance criteria:** A user should be able to upload a resume PDF, see the extracted text, edit the extracted text, and continue even if PDF parsing is imperfect.
...
**Acceptance criteria:** A user should be able to paste a job description, edit it before running analysis, and receive validation feedback if the input is too short.
...
**Strength levels:** Strong, Medium, Weak, Missing
...
**Scoring principle:** Scores should be explainable and based on requirement coverage.
...
**Score disclaimer:** The app should clearly communicate that this score is an estimated role alignment score based on the job description and resume evidence. It is not a guarantee of ATS performance, recruiter response, or hiring outcome.
...
- **Strong areas:** React; TypeScript; Production systems; Ownership; User-facing engineering work
- **Weak areas:** RAG experience; Formal LLM evaluation frameworks; Direct AI agent deployment; Customer-facing AI product delivery
- **Missing areas:** Tool calling; Vector databases; Prompt evaluation workflows
...
**Claims that require resume evidence:** The AI should flag a rewrite if it introduces new claims related to:

- New technologies, languages, frameworks
- New metrics, business impact, scale claims
- New AI/LLM techniques (RAG, tool calling, fine-tuning, vector databases, etc.)
- Production deployment, customer-facing work, leadership, architecture ownership
- Revenue impact, user count, performance/reliability improvements, cross-functional ownership
...
The app should flag this as **Risk level: High** — the rewrite introduces architecture ownership, AI experience, customer-facing scope, and a 100K+ user metric that are not supported by the original resume.
...
**AI:** OpenAI API, structured JSON outputs, multi-step prompting or server-side pipeline

**PDF parsing:** pdf-parse, pdfjs-dist, or any reliable PDF text extraction library

**Storage:** No database required for the MVP.

- React state for current session data
- Optional localStorage for preserving the latest analysis locally

Add a database later only if the product needs user accounts, saved resumes/descriptions, analysis history, shareable reports, or team/recruiter features.
...
**Input screen user flow:** Upload resume PDF → extract resume text → review and edit resume text → paste job description → Analyze → AI pipeline → Results screen.
...
2. Category scores — technical, experience, ownership, communication, AI/LLM (if relevant)
3. Requirement evidence map — requirement, evidence, strength, explanation
4. Gap analysis — strong, weak, missing areas
5. Bullet rewrite suggestions — original, improved, targeted requirements, risk, copy button
6. Factuality warnings — unsupported claims, claims requiring confirmation
7. Interview prep — likely questions, why they may ask, talking points
...
**Resume errors:** Not a PDF; PDF text extraction failed; extracted text too short; image-based or scanned PDF — user may need to paste resume text manually.

**Job description errors:** Too short; appears incomplete; not role-related text.

**AI errors:** Analysis failed; invalid structured output; token limit exceeded; analysis could not be completed.

**UI error example:** “We could not extract enough text from this PDF. Please paste your resume text manually or upload a text-based PDF.”
...
- Run an analysis
- See an overall match score and category-level fit scores
...
Every score, gap, and rewrite should be grounded in the user’s actual resume evidence or clearly marked as missing, weak, unsupported, risky, or needing user confirmation.
...
This repository implements the app with **Next.js**. From the project root:
...
Open [http://localhost:3000](http://localhost:3000). For production build: `npm run build` then `npm run start`.
